...
try:
    ...
    from bokeh.models import CustomJS, ColumnDataSource, Range1d, LabelSet, LinearColorMapper
except:
    print("Bokeh could not be loaded. Either it is not installed or you are not running within a notebook")


### CHNG - change all plot_width(plot_height) instances to width(height)
...
def kb_imshow(image, cmap='jet'):
    ...
    xr = Range1d(start=0, end=image.shape[1])
    yr = Range1d(start=image.shape[0], end=0)
    p = bpl.figure(x_range=xr, y_range=yr, tools="lasso_select")
...
def nb_plot_contour(image, A, d1, d2, thr=None, thr_method='max', maxthr=0.2, nrgthr=0.9,
                    face_color=None, line_color='red', alpha=0, line_width=2,
...
def nb_pick_dots(image, A, d1, d2, thr=None, thr_method='max', maxthr=0.2, nrgthr=0.9,
    ...
    """Plot dots and then lasso or polygon select them and send their idxs to a variable.
    ...
    Args:
        ...
        bg_brightness:
                number from 0-1, how bright of a white do you want on the bg image.
                too bright and you can't see the ROI markers well
    ...
    close_window_callback = CustomJS(code="""
    ...
    #s1.selected.js_on_change('indices', callback2)

    p.js_on_event("tap", close_window_callback)
    #p.state.on_session_destroyed(destroyed)
...
def nb_show_work(image, A, d1, d2, thr=None, thr_method='max', maxthr=0.2, nrgthr=0.9,
                    face_color=None, line_color='cyan', dot_color = 'blue', alpha=0.4,
